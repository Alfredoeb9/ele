    ...
    .mutation(async ({ ctx, input }) => {
      try {
        await ctx.db
          ...
          .where(
            and(
              eq(teamMembersTable.teamId, input.teamId),
              eq(teamMembersTable.userId, input.userEmail),
            ),
          );
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const isUserActive = await ctx.db
          .select()
          .from(users)
          .where(eq(users.username, input.inviteeUserName));
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction(async (tx) => {
        try {
          await tx
            .delete(teamMembersTable)
            .where(eq(teamMembersTable.teamId, input.teamId));
          await tx.delete(teams).where(eq(teams.id, input.teamId));
          await tx
            .delete(teamRecordTable)
            .where(eq(teamRecordTable.teamId, input.teamId));
