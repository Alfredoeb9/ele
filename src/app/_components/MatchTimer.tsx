...
export default function MatchTimer({ d2, d1, color }: MatchTimerTypes) {
  ...
  return (
    <>
      {d2.valueOf() <= d1.valueOf() ? (
        <span className={`text-xl sm:text-2xl ${color}`}>
          {(days as number) > 0 && days + "D"} {hours > 0 && hours + "H"}{" "}
          {minutes > 0 && minutes + "M"} {seconds > 0 && seconds + "S"}
        </span>
      ) : (
        <span className={`text-xl sm:text-2xl ${color}`}>Match Started</span>
      )}
    </>
  );
}
